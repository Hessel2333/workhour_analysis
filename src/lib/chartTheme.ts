...
function themeAxis(
  ...
  if (Array.isArray(axis)) {
    return axis.map((item) => themeAxis(item, textColor, lineColor, isCompact));
  }
...
export function withChartTheme(
  ...
  return {
    ...
    legend,
    xAxis: themeAxis(option.xAxis, textColor, lineColor, isCompact),
    yAxis: themeAxis(option.yAxis, textColor, lineColor, isCompact),
    grid: {
      bottom: isCompact ? (hasBottomLegend ? 58 : 34) : 40,
      left: isCompact ? 36 : 45,
      right: 25,
      top: isCompact ? (hasTopLegend ? 60 : 32) : 45,
      containLabel: true,
      ...(option.grid ?? {}),
    },
  };
