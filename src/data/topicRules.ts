...
export const TOPIC_RULES: TopicRule[] = [
  ...
  { label: '会议', keywords: ['会议', '讨论', '沟通', '整理', '大会', '过会', '开会'] },
  { label: '学习', keywords: ['学习', '研究', '调研', 'openclaw', 'ai生成', '分析方法'] },
  { label: '文档', keywords: ['文档', '任务书', '方案', '报告', 'pdf', '填报'] },
  { label: '运营', keywords: ['运营', '数据', '公众号', '推荐服务'] },
  { label: '现场支持', keywords: ['现场驻点', '驻点', '现场', '委外管理'] },
  { label: '合规管理', keywords: ['合规', '委外', '审核', '许可', '复垦', '水保', '能评', '环评'] },
];

const FALLBACK_SPLIT = /[、，,+/.\n\s()（）_-]+/;
const PENDING_PATTERNS = [/^\d{4}-\d{1,2}-\d{1,2}任务$/];

const DIRECT_LABELS: Array<{ label: string; confidence: number; keywords: string[] }> = [
  {
    label: '开发',
    confidence: 0.9,
    keywords: [
